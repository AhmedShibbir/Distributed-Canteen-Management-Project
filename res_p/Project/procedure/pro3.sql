create or replace procedure top_booking_customer(givenDate in VARCHAR)
	...
	cursor DC_cur is SELECT C_NAME , count(c_name) FROM customer1 @ site1 c ,booking1 @ site1 r where c.c_id=r.c_id  
	group by(c_name) order by count(c_name) DESC;
	cursor DC_cur1 is SELECT C_NAME , count(c_name) FROM customer2 @ site2 c ,booking2 @ site2 r where c.c_id=r.c_id 
	group by(c_name)  order by count(c_name) DESC;
begin
