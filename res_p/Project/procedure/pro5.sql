create or replace procedure all_branch_selling(s in varchar)
	...
	sumation number;
	cursor B_cur is ((SELECT  sum(price)  FROM menu1 @ site1 MI  INNER JOIN orders1 @ site1 as O ON MI.m_id = O.m_id
	                 WHERE O.b_id=1 ) union 
	                 ( SELECT  sum(price) FROM menu2 @ site2  MI  INNER JOIN orders2 @ site2 as O ON MI.m_id = O.m_id
	                 WHERE O.b_id=2 ));
begin
