...
create or replace procedure ordering_list(fromDate varchar,toDate varchar)
	...
	cursor B_cur is ( SELECT m_name  FROM  menu1  @ site1  MI INNER JOIN orders1 @ site1 O ON MI.m_id = O.m_id 
	                 WHERE O.o_date between to_Date(fromDate,'YYYY-MM-DD' ) AND to_date(toDate,'YYYY-MM-DD') 
					   )union 
	               ( SELECT m_name  FROM  menu2  @ site2  MI  INNER JOIN orders2   @ site2  O  ON MI.m_id = O.m_id
	                 WHERE O.o_date between to_Date(fromDate,'YYYY-MM-DD') AND to_date(toDate,'YYYY-MM-DD'));
